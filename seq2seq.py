...
class EncoderDecoderWithAttention(nn.Module):
    ...
    def __init__(self, *,
        ...
        self.encoder: Encoder = Encoder(input_size=embedding_size,
    ...
    def forward(self, *,
        ...
        verify_shape(tensor=input_embeddings, expected=[batch_size, input_seq_len, self.embedding.embedding_dim])

        encoder_states: torch.Tensor = self.encoder(input_tensor=input_embeddings,
        ...
        for t in range(output_seq_len):
            ...
            verify_shape(tensor=decoder_hidden_state, expected=[batch_size, 1, self.decoder.hidden_size])
            ...
            decoded_token: torch.Tensor = self.embedding(decoded_token_ids).unsqueeze(dim=1)
            ...
            decoder_results: Tuple[torch.Tensor, torch.Tensor] = self.decoder(batch_size=batch_size,
                                                                              ...
                                                                              hidden_tensor=decoder_hidden_state)
        ...
        permuted_result: torch.Tensor = result.permute(1, 0, 2)
